...
import { PurchaseItem, SaleItem, PublicExpenseItem, PrivateWalletTransaction, DealerStatementItem, DailyGoldPrices } from "../types";
...
interface DashboardOverviewProps {
  ...
  isArabic: boolean;
  goldOpeningBalance: number;
  onUpdateOpeningGold: (newVal: number) => void;
...
export default function DashboardOverview({
  ...
  // Quick Direct Gold Opening Stock Overrides
  const [isOverridingGold, setIsOverridingGold] = useState<boolean>(false);
  const [newGoldOpeningInput, setNewGoldOpeningInput] = useState<string>("");

  const handlePerformGoldOverride = () => {
    const targetVal = Number(newGoldOpeningInput);
    if (isNaN(targetVal) || targetVal < 0) {
      showAlert(isArabic ? "برجاء توفير قيمة صحيحة لوزن الذهب" : "Please provide a valid numeric gold weight.");
      return;
    }

    onUpdateOpeningGold(targetVal);
    setNewGoldOpeningInput("");
    setIsOverridingGold(false);
    showAlert(isArabic ? "تم تعديل وتسوية الرصيد الافتتاحي العيني للذهب بنجاح!" : "Gold opening balance successfully updated and matched!");
  };
  ...
  const t = {
    ...
    goldVaultTitle: isArabic ? "خزنة الذهب العينية (عيار ٢١ مكافئ)" : "Physical Gold Vault (21k Equiv)",
    availableGold: isArabic ? "الذهب الموجود حالياً بالخزنة" : "Available Gold in Stock",
    totalSoldGold: isArabic ? "إجمالي الذهب المباع" : "Total Gold Sold",
    totalPurchasedGold: isArabic ? "إجمالي الذهب المشترى والوارد" : "Total Incoming Gold (Buys)",
    estimatedVaultValue: isArabic ? "القيمة التقديرية للرصيد المتاح" : "Est. Capital Market Value",
  ...
  // 5. Gold Vault Calculations (عيار 21 المرجعي)
  // Incoming Gold from Purchases:
  const totalPurchasesGold21 = purchases.reduce((acc, p) => acc + p.equivalentWeight21, 0);
  const totalPurchasesGoldValueReal = purchases.reduce((acc, p) => acc + p.goldValue, 0);

  // Incoming Gold from Dealers (Type: gold_received_from_dealer in Statements):
  const totalDealersReceivedGold21 = dealerStatements
    .filter((ds) => ds.type === "gold_received_from_dealer")
    .reduce((acc, ds) => acc + Math.abs(ds.equivalentWeight21), 0);
  const totalDealersReceivedGoldValueReal = dealerStatements
    .filter((ds) => ds.type === "gold_received_from_dealer")
    .reduce((acc, ds) => acc + Math.abs(ds.goldValue), 0);

  // Total Incoming Gold (Grams & value)
  const totalIncomingGold21Combined = totalPurchasesGold21 + totalDealersReceivedGold21;
  const totalIncomingGoldValueRealCombined = totalPurchasesGoldValueReal + totalDealersReceivedGoldValueReal;

  // Outgoing Gold (Sold) via Sales Tab:
  const totalWeightSoldTab = sales.reduce((acc, s) => acc + s.equivalentWeight21, 0);
  const totalSalesGoldValueTab = sales.reduce((acc, s) => acc + s.goldValue, 0);

  // Outgoing Gold (Sold/Delivered) manually to Dealers (Type: gold_sold_to_dealer in Statements, excluding ds_sale_ entries):
  const totalWeightSoldDealerManual = dealerStatements
    .filter((ds) => ds.type === "gold_sold_to_dealer" && !ds.id.startsWith("ds_sale_"))
    .reduce((acc, ds) => acc + Math.abs(ds.equivalentWeight21), 0);
  const totalSalesGoldValueDealerManual = dealerStatements
    .filter((ds) => ds.type === "gold_sold_to_dealer" && !ds.id.startsWith("ds_sale_"))
    .reduce((acc, ds) => acc + Math.abs(ds.goldValue), 0);

  // Total Sold Gold (Grams & value combined)
  const totalSoldGold21Combined = totalWeightSoldTab + totalWeightSoldDealerManual;
  const totalSoldGoldValueRealCombined = totalSalesGoldValueTab + totalSalesGoldValueDealerManual;

  // Available Gold in Vault: (Opening gold balance + Incoming gold) - Sold gold
  const availableGold21Combined = (goldOpeningBalance + totalIncomingGold21Combined) - totalSoldGold21Combined;
  const availableGoldEstimatedValue = Math.round(availableGold21Combined * goldPrices.gold21);
  ...
  return (
    <div id="gold-accounting-dashboard-overview" className="space-y-6 animate-fade-in text-slate-100" dir={isArabic ? "rtl" : "ltr"}>
      ...
      {/* SECTION: PHYSICAL GOLD VAULT SECTION */}
      <div id="gold-vault-section" className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-3 gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-black text-white">{t.goldVaultTitle}</h2>
              <p className="text-[10px] text-slate-400 mt-0.5">
                {isArabic ? "جرد موازين الذهب والوارد والمباع الفعلي بالورشة أحمد وأبرش لتغطية الصك" : "Live weight ledger audit trail and physical stock controls"}
              </p>
            </div>
          </div>
          
          <div className="text-[10px] bg-slate-900 border border-slate-800 rounded px-2.5 py-1 text-slate-400 font-mono">
            {isArabic ? "السعر الاسترشادي لجرام ٢١ اليوم هو:" : "Karat 21 standard price reference today:"}{" "}
            <span className="text-amber-400 font-bold">{formatCurrency(goldPrices.gold21, isArabic)}</span>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          
          {/* CARD 1: AVAILABLE GOLD (الموجود حاليا) */}
          <div className="bg-slate-900 border border-amber-500/30 rounded-xl p-5 shadow flex flex-col justify-between relative overflow-hidden">
            <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-amber-500/10 to-transparent rounded-full -mr-6 -mt-6 pointer-events-none" />
            <div className="flex justify-between items-start">
              <div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">{t.availableGold}</p>
                {!isOverridingGold ? (
                  <div className="flex items-center gap-2 mt-2">
                    <h3 className="text-xl font-black text-amber-450 font-mono">
                      {formatWeight(availableGold21Combined, isArabic)}
                    </h3>
                    <button
                      onClick={() => {
                        setNewGoldOpeningInput(goldOpeningBalance.toString());
                        setIsOverridingGold(true);
                      }}
                      className="p-1 text-slate-400 hover:text-amber-400 hover:bg-slate-800 rounded transition-colors"
                      title={isArabic ? "تعديل وضبط الرصيد الافتتاحي للذهب" : "Adjust / Override gold opening balance"}
                    >
                      <Edit className="w-3.5 h-3.5" />
                    </button>
                  </div>
                  ...
                  <div className="mt-2 space-y-2">
                    <label className="text-[10px] text-amber-400 block font-bold">
                      {isArabic ? "الرصيد الافتتاحي الجديد (جرام عيار ٢١):" : "New Gold Opening (grams 21k equivalent):"}
                    </label>
                    <div className="flex items-center gap-1.5">
                      <input
                        type="number"
                        step="0.001"
                        value={newGoldOpeningInput}
                        onChange={(e) => setNewGoldOpeningInput(e.target.value)}
                        className="w-full max-w-[120px] bg-slate-950 border border-amber-500/50 rounded px-2 py-1 text-xs text-white font-mono focus:outline-none"
                        placeholder="e.g. 100.0"
                      />
                      <button
                        onClick={handlePerformGoldOverride}
                        className="p-1 bg-emerald-500 hover:bg-emerald-600 rounded text-slate-950 transition-colors"
                      ...
                      <button
                        onClick={() => setIsOverridingGold(false)}
                        className="p-1 bg-slate-800 hover:bg-slate-700 rounded text-slate-400 transition-colors"
                        title={isArabic ? "إلغاء الأمر" : "Cancel"}
                      >
                        <X className="w-3.5 h-3.5" />
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 flex justify-between text-[10px] text-slate-500 font-semibold">
              <span>{t.estimatedVaultValue}</span>
              <span className="font-mono text-emerald-400">{formatCurrency(availableGoldEstimatedValue, isArabic)}</span>
            </div>
          </div>

          {/* CARD 2: SOLD GOLD (المباع) */}
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">{t.totalSoldGold}</p>
                <h3 className="text-xl font-black text-rose-400 mt-2 font-mono">
                  {formatWeight(totalSoldGold21Combined, isArabic)}
                </h3>
              </div>
              <span className="p-2.5 bg-rose-500/10 rounded-lg text-rose-400 border border-rose-500/20 flex-shrink-0">
                <TrendingUp className="w-5 h-5 animate-pulse" />
              </span>
            </div>
            <div className="mt-4 pt-3 border-t border-slate-800 flex justify-between text-[10px] text-slate-500">
              <span>{isArabic ? "إجمالي القيمة الفورية للمبيعات" : "Total immediate sold cash-equivalent value"}</span>
              <span className="font-mono text-slate-300 font-bold">{formatCurrency(totalSoldGoldValueRealCombined, isArabic)}</span>
            </div>
          </div>

          {/* CARD 3: TOTAL PURCHASED / RECEIVED INFLOW GOLD (الوارد الكلي للمخزن) */}
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">{t.totalPurchasedGold}</p>
                <h3 className="text-xl font-black text-emerald-400 mt-2 font-mono">
                  {formatWeight(totalIncomingGold21Combined, isArabic)}
                </h3>
              </div>
              <span className="p-2.5 bg-emerald-500/10 rounded-lg text-emerald-400 border border-emerald-500/20 flex-shrink-0">
                <Scale className="w-5 h-5" />
              </span>
            </div>
            <div className="mt-4 pt-3 border-t border-slate-800 flex justify-between text-[10px] text-slate-500">
              <span>{isArabic ? "مصدرها: الشراء من الصاغة والتوريد" : "Sourced from customer buys & dealer inflows"}</span>
              <span className="font-mono text-slate-300 font-bold">+{formatWeight(totalPurchasesGold21, isArabic)}</span>
            </div>
          </div>

        </div>

        {/* VAULT RATIOS AND FORMULAS TRIBUTE INFO BLOCK */}
        <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800/80 text-[11px] text-slate-400 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="px-1.5 py-0.5 text-[9px] bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded font-black font-mono">
              AUDIT FORMULA
            </span>
            <span>
              {isArabic 
                ? "معادلة جرد الذهب: الذهب المتاح = (الرصيد الافتتاحي + إجمالي مشتريات الورشة من العملاء أحمد وأبرش + المسترجع من التجار) - إجمالي المبيعات الفورية وتنزيل سبيكة التاجر" 
                : "Vault formula: Available = (Opening stock + Purchased from clients أحمد & customers + Received from dealers) - Sold to dealers & direct clients."}
            </span>
          </div>
          <div className="font-mono text-[10.5px] text-amber-400 font-bold bg-slate-950 px-2 py-0.5 rounded border border-slate-800/80 whitespace-nowrap">
            {isArabic ? "افتتاحي الخزنة الحالي:" : "Current opening balance:"} {formatWeight(goldOpeningBalance, isArabic)}
          </div>
        </div>
      </div>
