...
export default function SalesManager({
  ...
  const [cashReceived, setCashReceived] = useState(""); // cash paid immediately on the spot by dealer
  ...
  const t = {
    ...
    outstandingLoan: isArabic ? "حساب التاجر قبل التصفية كاش:" : "Dealer cash balance before sale:",
  ...
  let unpaidLoanBeforeSale = 0;
  activeDealerStatements.forEach((item) => {
    if (item.type === "loan_received") {
      unpaidLoanBeforeSale += item.cashAmount;
    } else if (item.type === "loan_paid_cash") {
      unpaidLoanBeforeSale += item.cashAmount; // which is negative
    } else if (item.type === "gold_sold_to_dealer") {
      unpaidLoanBeforeSale += (-item.goldValue + item.cashAmount);
  ...
  // After offset:
  let remainingLoanAfterSale = 0;
  let remainingReimbursementFromDealer = 0;

  if (unpaidLoanBeforeSale >= 0) {
    // We owe the dealer (previous cash loan)
    remainingLoanAfterSale = Math.max(0, unpaidLoanBeforeSale - (calculatedGoldValue - numericCashReceived));
    remainingReimbursementFromDealer = Math.max(0, (calculatedGoldValue - numericCashReceived) - unpaidLoanBeforeSale);
  } else {
    // The dealer already owes us cash (previous receivable)
    remainingLoanAfterSale = 0;
    remainingReimbursementFromDealer = Math.abs(unpaidLoanBeforeSale) + (calculatedGoldValue - numericCashReceived);
  }
        ...
        <div className="lg:col-span-8 bg-slate-900 border border-amber-500/20 rounded-xl p-5 shadow-lg">
          ...
          <form onSubmit={handleRegisterSale} className="space-y-4 text-xs font-semibold text-slate-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block mb-1 text-slate-400">{isArabic ? "المقبوض كاش فوراً" : "Cash Recvd on Spot"}</label>
                <div className="relative">
                  <span className="absolute left-2.5 top-2.5 text-[9px] text-slate-500 font-bold">EGP</span>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-medium text-slate-300">
                <div className="space-y-1.5">
                  ...
                  {numericCashReceived > 0 && (
                    <div className="flex justify-between text-cyan-400">
                      <span>{isArabic ? "المسدد نقداً:" : "Paid Immediately:"}</span>
                      <span className="font-mono font-bold">{formatCurrency(numericCashReceived, isArabic)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-1.5 border-t sm:border-t-0 sm:border-r border-slate-800 pt-3 sm:pt-0 sm:pr-4">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-mono font-bold ${unpaidLoanBeforeSale >= 0 ? "text-rose-450" : "text-emerald-400"}`}>
                      {unpaidLoanBeforeSale >= 0 
                        ? formatCurrency(unpaidLoanBeforeSale, isArabic)
                        : (isArabic ? `له طرفنا ${formatCurrency(Math.abs(unpaidLoanBeforeSale), isArabic)}` : `Owes us ${formatCurrency(Math.abs(unpaidLoanBeforeSale), isArabic)}`)}
                    </span>
                  </div>
                  ...
                  {unpaidLoanBeforeSale > 0 && (
                    remainingLoanAfterSale > 0 ? (
                      ...
                      <div className="flex justify-between text-emerald-400 font-bold">
                        <span>{isArabic ? "تسوية السلفة المتبقية:" : "Repaid Loan:"}</span>
                  ...
                  {remainingReimbursementFromDealer > 0 && (
                    <div className="flex justify-between text-emerald-400 font-black bg-emerald-500/10 p-1 rounded border border-emerald-500/20 mt-1">
                      <span>{unpaidLoanBeforeSale >= 0 ? t.remainingDue : (isArabic ? "إجمالي مستحقاتنا طرف التاجر:" : "Total balance due to us:")}</span>
            ...
            <table className="w-full text-right text-xs">
              <thead className="bg-slate-950 text-slate-400 font-bold border-b border-slate-800">
                <tr>
                  ...
                  <th className="p-3 text-center">{isArabic ? "المستلم كاش" : "Cash Received"}</th>
                ...
                {filteredSales.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="p-3 text-center font-mono text-cyan-400 font-bold">{s.cashReceived ? formatCurrency(s.cashReceived, isArabic) : (isArabic ? "٠ ج.م" : "0 EGP")}</td>
